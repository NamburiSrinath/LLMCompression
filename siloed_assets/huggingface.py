...
# TODO - Added EMNLP code here
def get_quantized_layers(model, quantization_type, no_of_layers=12):
    # 1. RUN EXPS with pruning just encoder att, dec att
    # 2. RUN EXPS with pruning just self att, cross att
    if quantization_type == 'attention_only':
        quantize_layers = []
        for i in range(no_of_layers):
            # quantize_layers.append(f'encoder.block.{i}.layer.0.SelfAttention.q')
            # quantize_layers.append(f'encoder.block.{i}.layer.0.SelfAttention.k')
            # quantize_layers.append(f'encoder.block.{i}.layer.0.SelfAttention.v')
            # quantize_layers.append(f'encoder.block.{i}.layer.0.SelfAttention.o')

            quantize_layers.append(f'decoder.block.{i}.layer.0.SelfAttention.q')
            quantize_layers.append(f'decoder.block.{i}.layer.0.SelfAttention.k')
            quantize_layers.append(f'decoder.block.{i}.layer.0.SelfAttention.v')
            quantize_layers.append(f'decoder.block.{i}.layer.0.SelfAttention.o')
            quantize_layers.append(f'decoder.block.{i}.layer.1.EncDecAttention.q')
            quantize_layers.append(f'decoder.block.{i}.layer.1.EncDecAttention.k')
            quantize_layers.append(f'decoder.block.{i}.layer.1.EncDecAttention.v')
            quantize_layers.append(f'decoder.block.{i}.layer.1.EncDecAttention.o')
        quantize_layers = set(quantize_layers)

    # if quantization_type == 'output_only':
    #     quantize_layers = bart_quantize_output_linear_layers(model)
    #     quantize_layers = set(quantize_layers)
    
    # if quantization_type == 'all_layers':
    #     quantize_layers = {torch.nn.Linear}
    print(f"No of layers quantized is: {len(quantize_layers)}")
...
class HuggingFaceAutoLM(BaseLM):
    ...
    def __init__(
        ...
        # TODO - Added EMNLP code here
        print(f"Quantization flag in init {quantization_flag}")
        if quantization_flag == "None":
            ...
            basemodel = self._create_auto_model(
                pretrained=pretrained,
                quantized=quantized,
                trust_remote_code=trust_remote_code,
                revision=revision,
            ...
            print("Basemodel is --------")
            print(basemodel)
            quantize_layers = get_quantized_layers(basemodel, quantization_type=quantization_flag, no_of_layers=no_of_layers)
            self.model = torch.quantization.quantize_dynamic(basemodel, quantize_layers, dtype=torch.qint8)
            print(f"Quantized model is with flag {quantization_flag}--------")
            print(self.model)
